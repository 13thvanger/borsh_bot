...
async def telegram_photo_to_data_url(bot: Bot, file_id: str) -> str:
    ...
    mime_type = mimetypes.types_map.get(suffix.lower(), "image/jpeg")
    encoded = base64.b64encode(data).decode("utf-8")
    return f"data:{mime_type};base64,{encoded}"


async def check_borsh_image(data_url: str) -> tuple[bool, float, str, str]:
    headers = {
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT},
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "Проверь фото. Это борщ? Ответь только JSON."},
    ...
    async with httpx.AsyncClient(timeout=settings.agent_timeout_seconds) as client:
        response = await client.post(settings.agent_url, headers=headers, json=payload)
    ...
    return is_borsh, confidence, reason, content
